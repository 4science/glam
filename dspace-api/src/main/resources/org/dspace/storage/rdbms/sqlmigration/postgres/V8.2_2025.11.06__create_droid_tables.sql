...
BEGIN
   IF NOT EXISTS (
      ...
      WHERE table_name = 'most_recent_checksum'
      AND constraint_name = 'most_recent_checksum_pk'
   ) THEN
      RAISE INFO 'Creating most_recent_checksum_pk CONSTRAINT on most_recent_checksum';
...
-- the initial results
insert into droid_check_status(status_code, result_description)
select 'NOT_PROCESSED', 'The droid validation was not applied'
WHERE NOT EXISTS (
	SELECT 1
...
insert into droid_check_status(status_code, result_description)
SELECT 'NOT_FOUND', 'The bitstream is not complaint with any droid registered format'
WHERE NOT EXISTS (
	SELECT 1
	FROM droid_check_status dcs
...
insert into droid_check_status(status_code, result_description)
select 'PROCESSED', 'The bitstream has been processed with the droid validation'
WHERE NOT EXISTS (
	SELECT 1
	FROM droid_check_status dcs
...
insert into droid_check_status(status_code, result_description)
SELECT 'VALIDATED', 'Bitstream has been validated correctly with droid'
WHERE NOT EXISTS (
	SELECT 1
	FROM droid_check_status dcs
...
insert into droid_check_status(status_code, result_description)
select 'PARTIAL_VALIDATION', 'Bitstream validation partially performed due to a lack of information'
WHERE NOT EXISTS (
	SELECT 1
	FROM droid_check_status dcs
...
insert into droid_check_status(status_code, result_description)
select 'VALIDATION_ERROR', 'Some errors occurred during the validation of the bitstream'
WHERE NOT EXISTS (
	SELECT 1
	FROM droid_check_status dcs
