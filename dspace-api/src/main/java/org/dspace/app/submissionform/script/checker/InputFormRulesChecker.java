...
public class InputFormRulesChecker extends InputFormExcel {
    ...
    private static final String[] VISIBILITYSCOPE_VALUES = new String[] { "",
            "limited to submission", "limited to workflow",
            "limited to submission hidden", "limited to submission readonly",
            "limited to workflow hidden", "limited to workflow readonly",
            "submission hidden", "submission readonly",
            "workflow hidden", "workflow readonly", "readonly", "hidden" };

    private static final String[] INPUTTYPE_VALUES = new String[] { "onebox", "tag", "textarea", "name", "link",
            "date", "dropdown", "list", "qualdrop_value", "number", "series", "group", "inline-group", "calendar",
            "markdown", "openlist", "opendropdown", "geomap" };
    ...
    public List<InputFormErrorBuilder> check(File fileExcel, Context context) {
		...
		if (workbook != null) {
			...
			List<String> listNamesD = new ArrayList<String>();

			// list of dc element and dc qualifier in one form name (for duplicating check): no default value
			List<InputFormFieldElement> elements = new ArrayList<InputFormFieldElement>();

			// list of dc element and dc qualifier in one form name (for duplicating check) for default value
			List<InputFormFieldElement> duplicatedElements = new ArrayList<InputFormFieldElement>();

			// Form name list
			List<String> formNames = new ArrayList<String>();
			String lastFormName = "", currFormName = "";
			int lastPageNumber = 1, currPageNumber;
			...
			Cell[] listNames = sheet.getColumn(posListName), inputTypes = sheet.getColumn(posInputType);

			//all sheet rows (based on column 0)
			int rows = sheet.getColumn(0).length;

			//current list name and input type
			String listName, inputType;
			int indexRow;

			ArrayList<Cell[]> nestedRows = new ArrayList<Cell[]>();
			ArrayList<InputFormFieldElement> nestedEls = new ArrayList<InputFormFieldElement>();
			ArrayList<String> nestedFormNameList = new ArrayList<String>();

			ArrayList<Cell[]> groupFieldRows = new ArrayList<Cell[]>();
			ArrayList<InputFormFieldElement> groupFieldEls = new ArrayList<InputFormFieldElement>();
			...
			for (indexRow = 1; indexRow < rows; indexRow++) {
				...
				if (this.get(posFormName).equals("")) {
				    errorMessage = new StringBuilder().append(I18nUtil.getMessage("excel.to.inputform.check.formname.required", new Object[]{element.toString()}));
				...
				if (this.get(posRowNumber).equals("")) {
				    errorMessage = new StringBuilder().append(I18nUtil.getMessage("excel.to.inputform.check.row_number.required", new Object[]{element.toString()}));
				    InputFormErrorBuilder.manageError(errors, errorMessage);
				} else {
					try {
						Integer.parseInt(this.get(posRowNumber));
					} catch (NumberFormatException e) {
					    errorMessage = new StringBuilder().append(I18nUtil.getMessage("excel.to.inputform.check.row_number.valid", new Object[]{element.toString()}));
						InputFormErrorBuilder.manageError(errors, errorMessage);
					}
				}

				// No right repeat value set on cell
				if (!this.get(posParent).equals("") && this.get(posRepeatable).equals("true") && !this.get(posInputType).equals("list")) {
				    errorMessage = new StringBuilder().append(I18nUtil.getMessage("excel.to.inputform.check.repeatable.parent", new Object[]{element.toString()}));
				    InputFormErrorBuilder.manageError(errors, errorMessage);
				}

				// No right dc element set on cell
				if (this.get(posDcElement).equals("")) {
				    errorMessage = new StringBuilder().append(I18nUtil.getMessage("excel.to.inputform.check.dc_element.required", new Object[]{element.toString()}));
					InputFormErrorBuilder.manageError(errors, errorMessage);
				}

				// No right input type set on cell
				if (this.get(posInputType).equals("")) {
				    errorMessage = new StringBuilder().append(I18nUtil.getMessage("excel.to.inputform.check.input_type.required", new Object[]{element.toString()}));
					InputFormErrorBuilder.manageError(errors, errorMessage);
				}
				...
				if (StringUtils.isNotBlank(validationString)) {
				    try {
	                    ...
	                    errorMessage = new StringBuilder().append(I18nUtil.getMessage("excel.to.inputform.check.validation.invalid", new Object[]{element.toString(), validationString}));
	                    InputFormErrorBuilder.manageError(errors, errorMessage);
                    }
				}
				...
				if (labelValue.equals("")) {
					errorMessage = new StringBuilder().append(I18nUtil.getMessage("excel.to.inputform.check.label.required", new Object[]{element.toString()}));
					InputFormErrorBuilder.manageError(errors, errorMessage);
				}

				// No right repeat value set on cell
				if (this.get(posRepeatable).equals("")) {
				    errorMessage = new StringBuilder().append(I18nUtil.getMessage("excel.to.inputform.check.repeatable.valid", new Object[]{element.toString()}));
					InputFormErrorBuilder.manageError(errors, errorMessage);
				}
				...
				if (!this.get(posVisibility).equals("")
				    ...
				    errorMessage = new StringBuilder().append(I18nUtil.getMessage("excel.to.inputform.check.required.valid", new Object[]{element.toString()}));
					InputFormErrorBuilder.manageError(errors, errorMessage);
				}

				// No right scope set on cell
				if (!Arrays.asList(VISIBILITYSCOPE_VALUES).contains(this.get(posVisibility))) {
					errorMessage = new StringBuilder().append(I18nUtil.getMessage("excel.to.inputform.check.scope.valid",
				...
				if (!Arrays.asList(INPUTTYPE_VALUES).contains(this.get(posInputType))) {
					errorMessage = new StringBuilder().append(I18nUtil.getMessage(
							"excel.to.inputform.check.inputtype.valid", new Object[] { element.toString() }));
					InputFormErrorBuilder.manageError(errors, errorMessage);
				}
				...
				if (inputType.equals("tag")
				    ...
				    errorMessage = new StringBuilder().append(I18nUtil.getMessage("excel.to.inputform.check.repeatable.tag", new Object[]{element.toString()}));
					InputFormErrorBuilder.manageError(errors, errorMessage);
				}
				...
				if ((inputType.equals("group") || inputType.equals("inline-group"))
						&& !this.get(posRepeatable).equals("true")) {
				    errorMessage = new StringBuilder().append(I18nUtil.getMessage("excel.to.inputform.check.repeatable.group", new Object[]{element.toString()}));
					InputFormErrorBuilder.manageError(errors, errorMessage);
				}

				// if qualdrop_value no dc qualifier must be set
				if (inputType.equals("qualdrop_value")
						&& !this.get(posDcQualifier).equals("")) {
				    errorMessage = new StringBuilder().append(I18nUtil.getMessage("excel.to.inputform.check.posDcQualifier.qualdropvalue", new Object[]{element.toString()}));
					InputFormErrorBuilder.manageError(errors, errorMessage);
				}
				...
				if ((inputType.equals("dropdown")
						...
						&& listName.equals("")) {
				    errorMessage = new StringBuilder().append(I18nUtil.getMessage("excel.to.inputform.check.listname", new Object[]{element.toString(), inputType}));
					InputFormErrorBuilder.manageError(errors, errorMessage);
				}
				...
				if (!(inputType.equals("dropdown")
						...
						&& !listName.equals("")) {
				    errorMessage = new StringBuilder().append(I18nUtil.getMessage("excel.to.inputform.check.not.listname", new Object[]{element.toString(), inputType}));
					InputFormErrorBuilder.manageError(errors, errorMessage);
				}

				// No char _ in element or qualifier
				if (this.get(posDcElement).indexOf("_") != -1
						|| this.get(posDcQualifier).indexOf("_") != -1) {
				    errorMessage = new StringBuilder().append(I18nUtil.getMessage("excel.to.inputform.check.format.dc", new Object[]{element.toString(), inputType}));
					InputFormErrorBuilder.manageError(errors, errorMessage);
				}
				...
				if (!elements.contains(element)) {
				    ...
				    errorMessage = new StringBuilder().append(I18nUtil.getMessage("excel.to.inputform.check.form_name.checkduplicate", new Object[]{element.toString(), inputType}));
					InputFormErrorBuilder.manageError(errors, errorMessage);
				}
				...
				try {
					...
					if (!currFormName.equals(lastFormName)) {
						if (!formNames.contains(currFormName)) {
							...
							if (currPageNumber != 1) {
								errorMessage = new StringBuilder().append(I18nUtil.getMessage("excel.to.inputform.check.form_name.check.page", new Object[]{element.toString(), inputType}));
								InputFormErrorBuilder.manageError(errors, errorMessage);
							}
						} else {
						    errorMessage = new StringBuilder().append(I18nUtil.getMessage("excel.to.inputform.check.form_name.check", new Object[]{element.toString(), lastFormName}));
							InputFormErrorBuilder.manageError(errors, errorMessage);
						}
					} else {
						if (!(lastPageNumber == currPageNumber || lastPageNumber == currPageNumber - 1)) {
						    errorMessage = new StringBuilder().append(I18nUtil.getMessage("excel.to.inputform.check.form_name.check.all_rows", new Object[]{element.toString(), inputType}));
							InputFormErrorBuilder.manageError(errors, errorMessage);
						}
			...
			for (int i = 0; i < groupFieldRows.size(); i++) {
				...
				String groupFormName = groupElement.getFormName() + "-" + groupElement.getElementName().replace(".", "-");
				if (!nestedFormNameList.contains(groupFormName)) {
					errorMessage = new StringBuilder().append(I18nUtil.getMessage("excel.to.inputform.check.group.form_defined", new Object[]{groupElement.toString(), groupFormName }));
					InputFormErrorBuilder.manageError(errors, errorMessage);
				}
			}
			...
			for (int i = 0; i < nestedRows.size(); i++) {
				...
				if (!groupFieldList.contains(parentFieldName)) {
					errorMessage = new StringBuilder().append(I18nUtil.getMessage("excel.to.inputform.check.nested.parent_defined", new Object[]{nestedElement.toString(), parentFieldName }));
					InputFormErrorBuilder.manageError(errors, errorMessage);
				}
			}
			...
			while (iterListNamesD.hasNext()) {
				...
				if (!listNameOnValuePairSheet.contains(listaS) && !listaS.equals("")) {
				    errorMessage = new StringBuilder().append(I18nUtil.getMessage("excel.to.inputform.check.formvaluepairs", new Object[]{listaS}));
					InputFormErrorBuilder.manageError(errors, errorMessage);
				}
			}
