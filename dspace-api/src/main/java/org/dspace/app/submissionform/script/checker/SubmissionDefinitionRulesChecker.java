...
public class SubmissionDefinitionRulesChecker extends InputFormExcel {
	...
	public List<InputFormErrorBuilder> check(File fileExcel, Context context, String defaultDefinition) {
		...
		if (workbook != null) {
			...
			String lastDefinitionName = "", currDefinitionName = "";
				...
				try {
					...
					if (!currDefinitionName.equals(lastDefinitionName)) {
						if (!(definitionNameList.contains(currDefinitionName))) {
							...
							if (currStepOrder != 1) {
								errorMessage = new StringBuilder().append(I18nUtil.getMessage("excel.to.inputform.check.step.order.check.page", new Object[]{sheetRowNumber}));
								InputFormErrorBuilder.manageError(errors, errorMessage);
							}
						} else {
						    errorMessage = new StringBuilder().append(I18nUtil.getMessage("excel.to.inputform.check.step.order.check", new Object[]{sheetRowNumber, currDefinitionName}));
							InputFormErrorBuilder.manageError(errors, errorMessage);
						}
					} else {
						if (currStepOrder != 1 && lastStepOrder != (currStepOrder - 1)) {
						    errorMessage = new StringBuilder().append(I18nUtil.getMessage(
						    		"excel.to.inputform.check.step.order.check.all", new Object[]{sheetRowNumber}));
							InputFormErrorBuilder.manageError(errors, errorMessage);
						}
					}
				...
				for (indexRow = 1; indexRow < inputFormRows; indexRow++) {
					...
					if (sectionNames.get(i).contains(content[0])) {
						...
						if (metadataNamesGroupedBySubmission.get(i).contains(metadata)) {
							errorMessage = new StringBuilder().append(I18nUtil.getMessage(
									"excel.to.inputform.metadata.checkduplicate", new Object[]{definitionNameList.get(i), metadata}));
							InputFormErrorBuilder.manageError(errors, errorMessage);
						} else {
			...
			List<String> stepsOnStepsDefinitionSheet = new ArrayList<String>();
			...
			for (int i = 0; i < stepIdList.size(); i++) {
				...
				if (!stepsOnStepsDefinitionSheet.contains(stepIdEntry) && !stepIdEntry.equals("")) {
				    errorMessage = new StringBuilder().append(I18nUtil.getMessage(
				    		"excel.to.inputform.check.submission.step.not_valid", new Object[]{stepIdEntry}));
					InputFormErrorBuilder.manageError(errors, errorMessage);
				}
			}
		...
		for(InputFormErrorBuilder err : errors) {
		    System.out.println("LEVEL:" + err.getLevel() + " SHEET: submissions-definition" + " MESSAGE:"+ err.getErrorMsg());
		}
		return errors;
	}
