...
<xsl:stylesheet version="1.1"
    ...
    <xsl:template match="ScientificMaterial">
            ...
            <fo:page-sequence master-reference="simpleA4">
                <fo:flow flow-name="xsl-region-body">
                    ...
                    <xsl:call-template name="print-value">
                        <xsl:with-param name="label" select="'Description collection'" />
                        <xsl:with-param name="value" select="DescriptionCollection" />
                    </xsl:call-template>

                    <xsl:if test="IsPartOf/Item">
                        <fo:block font-size="10pt" margin-top="2mm">
                            <fo:inline font-weight="bold" text-align="right">
                                <xsl:text>Is part of: </xsl:text>
                            </fo:inline>
                            <fo:inline>
                                <xsl:for-each select="IsPartOf/Item">
                                    <xsl:value-of select="DisplayName" />
                                    <xsl:if test="position() != last()">, </xsl:if>
                                </xsl:for-each>
                            </fo:inline>
                    ...
                    <xsl:if test="HasPart/Item">
                        <fo:block font-size="10pt" margin-top="2mm">
                            <fo:inline font-weight="bold" text-align="right">
                                <xsl:text>Has part: </xsl:text>
                            </fo:inline>
                            <fo:inline>
                                <xsl:for-each select="HasPart/Item">
                                    <xsl:value-of select="DisplayName" />
                                    <xsl:if test="position() != last()">, </xsl:if>
                                </xsl:for-each>
                            </fo:inline>
                    ...
                    <xsl:call-template name="print-value">
                        <xsl:with-param name="label" select="'Coverage shelfmark'" />
                        <xsl:with-param name="value" select="CoverageShelfmark" />
                    </xsl:call-template>
