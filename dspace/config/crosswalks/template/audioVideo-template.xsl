        ...
        <fo:root xmlns:fo="http://www.w3.org/1999/XSL/Format">
            ...
            <fo:page-sequence master-reference="simpleA4">
                <fo:flow flow-name="xsl-region-body">
                    ...
                    <xsl:if test="CorporateBodies/CorporateBody">
                        <fo:block font-size="10pt" margin-top="2mm">
                            <fo:inline font-weight="bold" text-align="right">
                                <xsl:text>Corporate body(ies): </xsl:text>
                            </fo:inline>
                            <fo:inline>
                                <xsl:for-each select="CorporateBodies/CorporateBody">
                                    <xsl:value-of select="DisplayName" />
                                    <xsl:if test="position() != last()">, </xsl:if>
                                </xsl:for-each>
                            </fo:inline>
                    ...
                    <xsl:call-template name="print-value">
                        <xsl:with-param name="label" select="'Date issued'" />
                        <xsl:with-param name="value" select="DateIssued" />
                    </xsl:call-template>

                    <xsl:if test="AudioVideos/AudioVideo">
                        <fo:block font-size="10pt" margin-top="2mm">
                            <fo:inline font-weight="bold" text-align="right">
                                <xsl:text>Audio/Video: </xsl:text>
                            </fo:inline>
                            <fo:inline>
                                <xsl:for-each select="AudioVideos/AudioVideo">
                                    <xsl:value-of select="DisplayName" />
                                    <xsl:if test="position() != last()">, </xsl:if>
                                </xsl:for-each>
                            </fo:inline>
