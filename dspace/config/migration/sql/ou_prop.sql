...
FROM   ((SELECT DISTINCT crisid,
                         ...
                         0                    AS custompointer,
               old_cris_orgunit.sourceid,
               old_cris_orgunit.sourceref
         FROM   ( (SELECT *
                 FROM   old_cris_ou_no_prop) AS properties
                  LEFT JOIN (SELECT *
                             FROM   old_cris_ou_no) AS nested_object
                         ON nested_object.id = properties.parent_id
                  LEFT JOIN (SELECT *
                             FROM   old_cris_ou_no_pdef) AS property_def
                         ON property_def.id = properties.typo_id
                  LEFT JOIN (SELECT *
                             FROM   old_cris_ou_no_tp2pdef) AS tp2
                         ON tp2.cris_ou_no_tp_id = nested_object.typo_id
                            AND tp2.cris_ou_no_pdef_id != property_def.id
                  ...
                  LEFT JOIN (SELECT '#PLACEHOLDER_PARENT_METADATA_VALUE#' AS
                                    ...
                                    shortname,
                                    id
                             FROM   old_cris_ou_no_pdef) AS property_def2
                         ON property_def2.id = tp2.cris_ou_no_pdef_id
                            AND NOT EXISTS(SELECT *
                                           FROM   old_cris_ou_no_prop AS pdef3
                                           WHERE  pdef3.parent_id =
                                                  properties.parent_id
                                                  AND pdef3.typo_id =
                                                      property_def2.id)
                )
                LEFT JOIN old_cris_orgunit
                       ON nested_object.parent_id = old_cris_orgunit.id
                LEFT JOIN old_jdyna_values
                       ON old_jdyna_values.id = properties.value_id
         WHERE  property_def2.id IS NOT NULL)
        UNION
        (SELECT old_cris_orgunit.crisid,
                old_cris_ou_no_pdef.shortname,
                old_cris_ou_no.parent_id,
                old_cris_ou_no_prop.positiondef,
                old_cris_ou_no_prop.parent_id AS nested_object_id,
                visibility,
         ...
         FROM   old_jdyna_values
                JOIN old_cris_ou_no_prop
                  ON old_jdyna_values.id = old_cris_ou_no_prop.value_id
                JOIN old_cris_ou_no
                  ON old_cris_ou_no_prop.parent_id = old_cris_ou_no.id
                JOIN old_cris_orgunit
                  ON old_cris_ou_no.parent_id = old_cris_orgunit.id
                RIGHT JOIN old_cris_ou_no_pdef
                        ON old_cris_ou_no_pdef.id = old_cris_ou_no_prop.typo_id)
        UNION
        SELECT old_cris_orgunit.crisid,
               old_cris_ou_pdef.shortname,
               old_cris_ou_prop.parent_id,
               old_cris_ou_prop.positiondef,
               ...
               old_cris_orgunit.sourceref
        FROM   old_jdyna_values
               JOIN old_cris_ou_prop
                 ON old_jdyna_values.id = old_cris_ou_prop.value_id
               JOIN old_cris_orgunit
                 ON old_cris_ou_prop.parent_id = old_cris_orgunit.id
               JOIN old_cris_ou_pdef
                 ON old_cris_ou_pdef.id = old_cris_ou_prop.typo_id) AS
       innerquery
ORDER  BY crisid,
